...
| Sprint 9.5 | `POST /paiements/{id_paiement}/simuler-confirmation` déclenche, depuis l'écran de paiement, la confirmation qu'un vrai fournisseur enverrait normalement de lui-même par webhook. Aucune protection ne l'empêche de fonctionner si une vraie passerelle (Mvola, Stripe...) venait à remplacer `PasserelleSimulee` sans que l'endpoint soit retiré — décision actée en construisant 9.5, pour ne pas alourdir le sprint d'une garde défensive contre un futur qui n'existe pas encore. | Retirer cet endpoint (backend et bouton frontend) dès qu'une vraie passerelle est branchée — un vrai fournisseur confirme de lui-même, ce déclencheur manuel n'aurait alors plus de sens et deviendrait une porte dérobée pour confirmer un paiement sans jamais l'avoir réellement payé. **À traiter avant mise en prod.** |
