...
def seed_audit():
    ...
    try:
        ...
        # Persona de la prise de commande au comptoir (sprint 6). Un mot de
        # passe lui est posé : sans compte de connexion, elle ne pourrait pas
        # appeler `POST /commandes/personnel`, que `get_current_personnel`
        # protège. `est_administrateur` reste faux — prendre une commande n'est
        # pas un privilège d'administration.
        receptionniste = Personnel(
            nom="Randria",
            prenom="Hanta",
            fonction=FonctionPersonnel.RECEPTIONNISTE,
            est_administrateur=False,
            email=_email_unique("receptionniste"),
            telephone="032 7777777",
            date_embauche=datetime(2024, 2, 1),
            mot_de_passe=hacher_mot_de_passe("password123"),
        )
        db.add(receptionniste)
        ...
        # --- Hébergement lié à la formation (sprint 6) ---
        #
        # `avec_hebergement` n'est qu'un **souhait** : c'est
        # `#id_reservation_hebergement` qui porte la preuve qu'une chambre est
        # attribuée. Le couplage passe par **deux lignes** — le `CHECK`
        # d'exclusivité interdit qu'une même ligne porte `#id_session` et
        # `#id_logement`.
        #
        # Les dates sont celles de la session, comme le fait le service. Elles
        # ne recoupent pas celles de `reservation_logement` : la contrainte
        # d'exclusion refuserait deux réservations actives sur la même chambre.
        reservation_hebergement = Reservation(
            type_reservation=TypeReservation.LOGEMENT,
            date_debut=reservation_formation.date_debut,
            date_fin=reservation_formation.date_fin,
            nombre_personnes=reservation_formation.nombre_personnes,
            statut=StatutReservation.CONFIRMEE,
            id_client=client_particulier.id_client,
            id_logement=logement_disponible.id_logement,
        )
        db.add(reservation_hebergement)
        db.flush()
        reservation_formation.id_reservation_hebergement = (
            reservation_hebergement.id_reservation
        )

        # --- Commande saisie par un salarié (sprint 6) ---
        #
        # Le parcours que le sprint compose : le client réserve une table, puis
        # un salarié saisit sa commande. `#id_personnel` dit **qui** l'a prise ;
        # `NULL` sur les quatre autres commandes signifie qu'elles viennent du
        # parcours client.
        #
        # `Sur_place` et **aucune adresse** : c'est la présence de l'adresse, et
        # elle seule, qui déclenche une `LIVRAISON`. Il n'y en a donc pas ici.
        db.flush()
        commande_sur_place = Commande(
            type_commande=TypeCommande.SUR_PLACE,
            statut=StatutCommande.SERVIE,
            montant_total=Decimal("0.00"),
            id_client=client_particulier.id_client,
            id_reservation=reservation_table.id_reservation,
            id_personnel=receptionniste.id_personnel,
        )
        db.add(commande_sur_place)
        db.flush()
        db.add(
            LigneCommande(
                quantite=2,
                prix_unitaire_applique=eclair.prix_unitaire,
                id_commande=commande_sur_place.id_commande,
                id_produit=eclair.id_produit,
            )
        )
        commande_sur_place.montant_total = eclair.prix_unitaire * 2

        db.commit()
        print("✅ Seed audit visuel complété avec succès !")
        print("   - 2 clients (particulier + entreprise)")
        print("   - 3 personnels (formateur, livreur, réceptionniste)")
        print("   - 4 produits (nominal, rupture, personnalisable, non-livrable)")
        print("   - 3 salles (tarifs variés)")
        print("   - 3 logements (chaque statut)")
        print("   - 2 formations + 2 sessions")
        print("   - 4 commandes (tous statuts)")
        print("   - 3 livraisons (incluant Echouee)")
        print("   - 5 réservations (tous types + hébergement lié)")
        print("   - 1 commande sur place saisie par un salarié")
