...
def test_relancer_refuse_le_rejeu_apres_une_premiere_relance_reussie(
    ...
    """Distinct de `test_relancer_une_livraison_non_echouee_retourne_409` :
    celui-ci porte sur une livraison qui n'a jamais été `Echouee`, alors
    qu'ici elle l'a été puis a déjà été relancée avec succès."""
    client_http.put(
        f"{LIVRAISONS}/{id_livraison}/statut",
        json={"statut": "Echouee"},
        headers=entete_admin,
